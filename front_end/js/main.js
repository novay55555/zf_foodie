...
(function(w, $) {
    ...
    // 预先创建的webview, 用于首页的页面快速切换
    // 目前的webview管理策略是, 主页面使用预创建的id为main的webview通过loadURL切换
    // 如果需要其他webview, 则根据页面需求自行创建
    MyApp.preWebview = ['main'];
    ...
    MyApp.back = (function() {
        ...
        var back = function() {
            ...
            if (currentWebview.preload) {
                currentWebview.hide('auto');
                // 如果有额外的webview, 则删除
                setTimeout(function() {
                    if (allWebview.length > (MyApp.preWebview.length + 1)) {
                        allWebview.shift(); // 移除HBuilder
                        for (var i = 0, l = allWebview.length; i < l; i++) {
                            if (MyApp.preWebview.indexOf(allWebview[i].id) === -1) {
                                allWebview[i].close();
                            }
                        }
                    }
    ...
    /**
     * 常规的webview切换封装函数
     * 这种方法一般是调用hbuider提供的动画
     * 如果有特殊的需求, 需要自行实现
     * @param  {Object} wv       webview对象
     * @param  {String} url      页面路径
     * @param  {String} way      webview动画名
     * @param  {Number} duration webview动画的执行时间
     */
    MyApp.loadURL = function(wv, url, way, duration) {
        var animate = way || 'slide-in-right',
            time = duration || 200;
        if (animate === 'zoom-out') { // 如果webview本身有内容, zoom-out后会模糊
            wv.show(animate, time, function() {
                wv.loadURL(url);
            })
        } else {
            wv.loadURL(url);
            wv.onloaded = function() {
                wv.show(animate, time);
            };
        }
    ...
    // 页面初始化所需要的操作全部写在这里
    MyApp.init = function() {
        // Array.prototype.slice.call(document.querySelectorAll('section')).forEach(function(elem) {
    ...
    MyApp.ajax = {
        ...
        get: function(url, params, callback) {
            var waiting = plus.nativeUI.showWaiting('please wait...');
            var params = params || {},
                str = "?start=start";
            ...
            console.log('ajaxURL:' + this.baseURL + url + str);
            $.ajax({
                url: this.baseURL + url + str,
                async: true,
                type: "GET",
                dataType: "json",
...
;
(function(doc, win) {
    var evt = 'onrientationchange' in window ? 'onrientationchange' : 'resize',
        fn = function() {
            doc.documentElement.style.fontSize = 40 * document.documentElement.clientWidth / 320 + 'px';
        };
...
Foodie.prototype.init = function(dom, line) {
    ...
    for (var i = 0, arr = filterArr; i < filterLength; i++) {
        var eatingElem = arr.splice(Math.floor(filterArr.length * Math.random()), 1);
        if (i < line) { // 首行元素的样式
            eatingBoxHtml += '<span style="top:0;left:' + (perRem * i + 'rem;') + this.spanStyleStr(perRem) + '">' + eatingElem + '</span>';
        } else if (i < (2 * line - 2)) { // 首行和末行之间的右侧元素样式
            eatingBoxHtml += '<span style="right:0;bottom:' + (perRem * (2 * line - 2 - i) + 'rem;') + this.spanStyleStr(perRem) + '">' + eatingElem + '</span>';
        } else if (i < (3 * line - 2)) { // 末行元素样式
            eatingBoxHtml += '<span style="bottom:0;left:' + (perRem * (3 * line - 3 - i) + 'rem;') + this.spanStyleStr(perRem) + '">' + eatingElem + '</span>';
        } else { // 首行和末行之间的左侧元素样式
...
/**
 * 设置吃货内容span的样式
 * @param  {Number} rem rem的值
 * @return {String}     样式字符串
 */
Foodie.prototype.spanStyleStr = function(rem) {
    var r = ['width', 'height', 'line-height'].map(function(elem) {
        return elem + ':' + rem + 'rem;';
    });
    return r.join('');
};
